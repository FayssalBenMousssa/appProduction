...
public class DocumentResource {
    ...
    public void updateUserSequence(Document savedDocument) {
        ...
        Type type = savedDocument.getType();
        String userName = savedDocument.getLogs().get(0).getUserName();
        Document lastDocument =     documentRepository.findTopByTypeAndLogs_UserNameOrderByCodeDesc( type,  userName);


        if (user.getSettings()!= null && user.getSettings().size() > 0) {
            String[] parts = lastDocument.getCode().split("\\.");
            ...
            for (Setting setting : user.getSettings()) {
                ...
                if (Objects.equals(savedDocument.getType().getCode(), "bon_livraison") && settingName.equals("sequence_bon_livraison")) {
                    log.info("Updating sequence_bon_livraison setting with last part: " + lastPart);
                    ...
                    log.info("Updating sequence_bon_transfert setting with last part: " + lastPart);
                    setting.setValue(String.valueOf(sequenceNumber));
                } else if (Objects.equals(savedDocument.getType().getCode(), "prepa_bon_charge") && settingName.equals("sequence_prepa_bon_charge")) {
                    log.info("Updating sequence_prepa_bon_charge setting with last part: " + lastPart);
                    setting.setValue(String.valueOf(sequenceNumber));
                }
            }
    ...
    public void runSyncDocument(Document savedDocument) {
        ...
        try {
            log.info("runSync method called with document ID " + savedDocument.getId() + " and code " + savedDocument.getCode());
            if (!savedDocument.getIsSynchronised()) {
               ...
               // log.error(documentService.toOldJSON(savedDocument, user, payments));
                ...
                log.info("Document with ID " + savedDocument.getId() + " and code " + savedDocument.getCode() + " updated to isSynchronised = true");
            } else {
                log.info("Document with ID " + savedDocument.getId() + " and code " + savedDocument.getCode() + " already has isSynchronised = true");
            }
        } catch (Exception e) {
            log.error("document code {} id {} Error sending request to other server : {} " ,savedDocument.getCode() , savedDocument.getId(), e.getMessage());
        }
    }
